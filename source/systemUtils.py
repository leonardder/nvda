...
def areExecutablesRunning(*names) -> bool:
	"""Retrieves whether any process instances are running for the provided executable names."""
	import comtypes.client
	wmi = comtypes.client.CoGetObject("winmgmts:")
	# WMI requires paths with backslahses escaped
	escapedNames = [name.replace("\\", r"\\") for name in names]
	whereParts = " OR ".join(f"ExecutablePath = '{name}'" for name in escapedNames)
	query = f"SELECT ProcessId FROM Win32_Process WHERE {whereParts}"
	processes = wmi.ExecQuery(query)
	try:
		return bool(processes.Count)
