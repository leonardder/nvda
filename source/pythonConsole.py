...
class ConsoleUI(wx.Frame):
	...
	def onInputPaste(self, evt):
		...
		if not wx.TheClipboard.IsSupported(wx.DataFormat(wx.DF_TEXT)):
			evt.Skip()
			return
		cpTextData = wx.TextDataObject()
		if not wx.TheClipboard.Open():
			evt.Skip()
			return
		try:
			res = wx.TheClipboard.GetData(cpTextData)
		except:
			log.exception()
			evt.Skip()
			return
		...
		if not res:
			# Retrieval of text data failed
			evt.Skip()
			return
		cpText = cpTextData.GetText()
		if not cpText.strip():
			evt.Skip()
			return
		cpLines = cpText.split("\n")
		inputLine = self.inputCtrl.GetValue()
		pos = self.inputCtrl.GetInsertionPoint()
		prefix = inputLine[:pos]
		suffix = inputLine[pos:]
